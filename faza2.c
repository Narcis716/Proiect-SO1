...
static void notify_monitor(const char *district, int report_id)
{
    char log_msg[256];
    pid_t mon = read_monitor_pid();

    if (mon <= 0) {
        snprintf(log_msg, sizeof(log_msg),
                 "monitor_notify report_id=%d FAILED: no PID in %s",
                 report_id, MONITOR_PID);
        log_action(district, log_msg);
        return;
    }

    if (kill(mon, SIGUSR1) < 0) {
        snprintf(log_msg, sizeof(log_msg),
                 "monitor_notify report_id=%d FAILED: kill(%d) error: %s",
                 report_id, (int)mon, strerror(errno));
    } else {
        snprintf(log_msg, sizeof(log_msg),
                 "monitor_notify report_id=%d OK: SIGUSR1 sent to PID %d",
                 report_id, (int)mon);
    }
    log_action(district, log_msg);
}
...
void cmd_add(const char *district)
{
    ...
    notify_monitor(district, r.report_id);
